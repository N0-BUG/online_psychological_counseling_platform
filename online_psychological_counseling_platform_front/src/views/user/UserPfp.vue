...
const handleAvatarSuccess = function (response, uploadFile) {
  ...
  userPfp.value = response.data
};
const username = useRoute().params.username;

async function updatePfp() {
  try {
    const response = await axios.post('http://localhost:8091/user/updatePfp', {
      userPfp: userPfp.value,
      username: username,
    });
...
<template>
  <div class="container">
    <el-upload
        ...
        action="http://localhost:8091/user/upload"
